...
$fileRoot = "../"; //"http://jasmineenglish.com/demo/sousa/";
            ...
            for ($i = 0; $i < count($mapInstruments); $i++) {
                for ($f = 0; $f < count($mapFittings); $f++) {
                    if (!($mapModels[$m] == "master artisan" && $mapFittings[$f] == "nickel-silver")) {
                        $bowImage = "img/bow/thmb/" . str_replace(' ', '', strtolower($mapInstruments[$i])) . "_" . str_replace(' ', '', strtolower($mapModels[$m])) . "_" . str_replace(' ', '', strtolower($mapFittings[$f])) . ".png";
                        $fullBowImage = $fileRoot . $bowImage;
                        if (!file_exists($fullBowImage)) {
                            $bowImage = 'img/thumb_image-coming-soon.png';
                        }
                        $bow = "<div class='bow'>" .
                                "<a href='bow.php#instrument=$mapInstruments[$i]&amp;model=$mapModels[$m]&amp;fitting=$mapFittings[$f]&amp;view=0'>" .
